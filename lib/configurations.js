	...
	connection.connect(function(err){
		if (err){
			...
			async.waterfall([
				...
				deleteCgwsFromConfig,
				deleteSitesFromConfig,
				deleteConfig
			], function (err, result) {
				f({data: result});
			});
			...
			function selectDataFromConfig(callback) {
				...
				'SELECT e.idEMPLAZAMIENTO, c.idCGW ' +
				'FROM emplazamiento e ' +
				'LEFT JOIN cgw c ON e.idEMPLAZAMIENTO=c.EMPLAZAMIENTO_idEMPLAZAMIENTO ' +
				'WHERE cfg_idCFG =?', [cfg],
				function (err, result) {
					for (var i = 0; i < result.length; i++) {
						if (result[i].idEMPLAZAMIENTO != null)
							tempSites.push(result[i].idEMPLAZAMIENTO);
						if (result[i].idCGW != null)
							cgws.push(result[i].idCGW);
					}
					//Sacamos los id únicos de emplazamiento
					tempSites.forEach(function(element){
			...
			function deleteCgwsFromConfig(data, callback) {
				cgws.forEach(function(element) {
					var res = new Object();
					res.json = function(error, data){};
					myLibGateways.delGateway(null, res, element);
				});
				return callback(null, "OK");
			}
			function deleteSitesFromConfig(data, callback) {
				var queries = '';
				sites.forEach(function (item) {
					queries += mySql.format('DELETE FROM emplazamiento WHERE idEMPLAZAMIENTO = ? ;',
						[item]);
				});
			...
			function deleteConfig(data, callback) {
				var query = connection.query('DELETE FROM CFG WHERE idCFG = ?', cfg,
				function (err, result) {
					logging.LoggingDate(query.sql);
					if (err) {
						connection.end();
