...
/** 20170526 AGL Para las nuevas Estadisticas */
exports.getStatisticsSet = function (startTime, endTime, f) {
	var mySql = require('mySql');
	var connection = mySql.createConnection({
	  host     : gcfg.Ulises.dbhost,
	  user     : gcfg.Ulises.dbuser,
	...
	connection.connect(function(err){
		if (err) {
			var strErr = "Error de Acceso a Base de Datos: " + err.toString();
			logging.loggingError(strErr);
			return f(strErr);
		}
		var queryString = 'SELECT h.IdHw,h.TipoHw,h.IdIncidencia,DATE_FORMAT(h.FechaHora, "%d/%m/%Y %H:%i:%s") as FechaHora,' +
										'DATE_FORMAT(h.Reconocida, "%d/%m/%Y %H:%i:%s") as Reconocida,h.Descripcion,h.Usuario,I.LineaEventos as Alarma, I.Error as TipoAlarma ' +
										'FROM historicoincidencias h ,Incidencias I WHERE H.IdIncidencia=I.IdIncidencia AND h.FechaHora>=\'' + startTime + '\' AND h.FechaHora<=\'' + endTime + '\'  ' +
										'ORDER BY -h.FechaHora LIMIT ' + parseInt(gcfg.Ulises.dbstatreglimit);
		var query = connection.query(queryString, function(err, rows, fields) {
			connection.end();
			if (err) {
				var strErr = "Error en Query a Base de Datos: " + err.toString();
					return f(strErr);
				}
			logging.LoggingDate(query.sql);
			return f({error: err, howMany: rows.length, historics: rows});
		});
	});
};
