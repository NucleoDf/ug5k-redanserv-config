...
function updateServices4Copy(oldSite, newSite, f) {
	...
	connection.connect(function (err, usrs) {
		if (err) {
			...
			function createNewServices(updateArray, callback) {
				var queries = '';
			...
			function selectDataFromNewServices(data, callback) {
				var queries = '';
				updateArray.forEach(function (item) {
					queries += mySql.format('SELECT idSERVICIOS, name '+
											'FROM SERVICIOS WHERE idSERVICIOS = ',[data.insertId]);
				});
				connection.query(queries, function(err, results) {
					if (err) {
						connection.end();
