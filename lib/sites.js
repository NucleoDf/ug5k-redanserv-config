	...
	connection.connect(function(err, usrs){
		if (err){
				...
				function(err, rows, fields) {
					...
					if (err || rows.length == 0) {
						...
						for(var i=(allIpListInConfig.length / 2);i<allIpListInConfig.length;i++,j++) {
							...
							object2Insert.ipb = allIpListInConfig[i];
							object2Insert.name = rows[j].name;
			...
			function updateDataFromCpus(data2Insert, callback) {
				var query=connection.query('UPDATE cpu SET ipb=? WHERE idCPU=?', data2Insert[2].ipb, data2Insert[2].ipb,
					function(err, rows, fields) {
						logging.LoggingDate(query.sql);
						if (err || rows.length == 0){
