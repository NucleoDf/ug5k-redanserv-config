			...
			query = connection.query('INSERT INTO emplazamiento (cfg_idCFG,name) VALUES (?,?)', [idCfg, targetNameSite], function (err, result) {
				...
				query = connection.query('INSERT INTO cgw (EMPLAZAMIENTO_idEMPLAZAMIENTO,REGIONES_idREGIONES,servicio,name,dualidad,ipv,ips,nivelconsola,puertoconsola,nivelIncidencias) ' +
					...
					var _idCgw = result.insertId;
						...
						//AQUI VAMOS A HACER EL UPDATE!!!
						updateIpsAndNames(sourceIdSite, _idCgw);
...
function updateIpsAndNames(sourceIdSite, newIdCgw){
	var mySql = require('mySql');
	var connection = mySql.createConnection({
		host     : gcfg.Ulises.dbhost,
		user     : gcfg.Ulises.dbuser,
	...
	connection.connect(function(err, usrs){
		if (err){
			...
			function selectDataFromConfig(callback) {
				var query = connection.query('SELECT cgw.idCGW, cgw.name, c.ipb  '+
				'FROM cpu c '+
				'INNER JOIN cgw cgw ON cgw.idCGW = c.CGW_idCGW '+
				'INNER JOIN emplazamiento e ON e.idEMPLAZAMIENTO =  '+
				'cgw.EMPLAZAMIENTO_idEMPLAZAMIENTO AND e.idEMPLAZAMIENTO = ?', sourceIdSite,
				function(err, rows, fields) {
					...
					if (err || rows.length == 0) {
						...
						var allIpListInConfig = generateIps(rows);
						
						var j=0;
						for(var i=(allIpListInConfig.length / 2);i<allIpListInConfig.length;i++,j++) {
							var object2Insert = {};
							object2Insert.idCGW = newIdCgw;
							object2Insert.oldIpb = allIpListInConfig[j];
							object2Insert.newIpb = allIpListInConfig[i];
							object2Insert.name = rows[j].name+'(Copia)';
							data2Insert.push(object2Insert);
						}
						return callback(null, data2Insert);
					}
				});
			}
			function updateDataFromCpus(data2Insert, callback) {
				var query=connection.query('UPDATE cpu SET ipb=? WHERE CGW_idCGW=? AND ipb=?', [data2Insert[0].newIpb, data2Insert[0].idCGW, data2Insert[0].oldIpb],
					function(err, rows, fields) {
						logging.LoggingDate(query.sql);
						if (err || rows.length == 0){
			...
			function updateDataFromCgw(data2Insert, callback) {
				var query=connection.query('UPDATE cgw SET name=? WHERE idCGW=?', [data2Insert[0].name, data2Insert[0].idCGW],
					function(err, rows, fields) {
						logging.LoggingDate(query.sql);
						if (err || rows.length == 0){
							transaction.rollback();
...
function generateIps(ipList){
	var newList=[];
	var newIp="";
	var incIpDigit=0;
	
	for (var index in ipList) {
		newList.push(ipList[index].ipb);
	}
	...
	while(newList.length != 2*(ipList.length)) {
		do {
