	...
	connection.connect(function(err, usrs) {
		if (err) {
			...
			function selectDataFromNewConfig(callback) {
				var query=connection.query('SELECT DISTINCT CFG_idCFG, activa '+
											'FROM cgw_cfg '+
					...
					function(err, rows, fields) {
						...
						if (err || rows.length == 0){
							...
							configData.CFG_idCFG=rows[0].CFG_idCFG;
			...
			function updateEnlacePasarelaConfig(configData, callback) {
				var query=connection.query('UPDATE CGW_CFG SET CFG_idCFG=?, activa=? WHERE CGW_idCGW=?', [configData.CFG_idCFG, configData.activa, IdGateway],
					function(err, rows, fields) {
						logging.LoggingDate(query.sql);
						if (err || rows.length == 0){
							transaction.rollback();
...
exports.getGatewaysIpInConfig = function(ip, idCfg, idCgw, f){
	var mySql = require('mySql');
	var connection = mySql.createConnection({
		host     : 'localhost',
		user     : 'root',
