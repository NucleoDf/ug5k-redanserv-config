	...
	connection.connect(function(err){
		if (err){
			...
			var querySel = connection.query('SELECT c.idCGW FROM CGW c WHERE idCGW=? '+
				...
				//Llamamos a borrar los recursos
				deleteResources4Gateway(gtwBorrar, function (data) {
					var data = data;
				});
			...
			function selectIdsResourcesFromGtw(callback) {
				...
				function (err, rows) {
					...
					if (err) {
						...
						idSlaves[0] = rows[0].SLAVES_idSLAVES;
						idSlaves[1] = rows[1].SLAVES_idSLAVES;
						idSlaves[2] = rows[2].SLAVES_idSLAVES;
						idSlaves[3] = rows[3].SLAVES_idSLAVES;
						
						gtwBorrar.idSalves = idSlaves;
						return callback(null, gtwBorrar);
					}
				});
				/*function selectidResourcesFromGtw(gtwBorrar, callback) {
					var query = connection.query(
						'SELECT SLAVES_idSLAVES ' +
						'FROM hardware h ' +
						'LEFT JOIN slaves s ON h.SLAVES_idSLAVES = s.idSLAVES ' +
